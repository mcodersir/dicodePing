...
copy /y "app\build\outputs\apk\standard\release\app-standard-release.apk" "release\dicodePing-v1.9.0-rc.4-android.apk" >nul
if errorlevel 1 goto :fail

set "APKSIGNER="
for /f "delims=" %%F in ('where apksigner.bat 2^>nul') do if not defined APKSIGNER set "APKSIGNER=%%F"
if defined APKSIGNER (
  call "%APKSIGNER%" verify --verbose --print-certs "release\dicodePing-v1.9.0-rc.4-android.apk"
  if errorlevel 1 goto :fail
) else (
  echo Warning: apksigner was not found in PATH; Gradle signing completed but external verification was skipped.
)

certutil -hashfile "release\dicodePing-v1.9.0-rc.4-android.apk" SHA256 > "release\dicodePing-v1.9.0-rc.4-android.apk.sha256.txt"

echo.
echo Signed APK created:
echo   release\dicodePing-v1.9.0-rc.4-android.apk
