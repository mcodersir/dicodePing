...
if 'versionName = "1.9.0-rc.5"' not in build_file:
    errors.append("versionName must be 1.9.0-rc.5 for this release")
if 'buildConfigField("String", "RELEASE_VERSION", "\\"1.9.0-rc.5\\"")' not in build_file:
    errors.append("RELEASE_VERSION must be 1.9.0-rc.5 for this release")

if "compileSdk = 36" not in build_file or "targetSdk = 36" not in build_file:
    errors.append("Android RC5 must compile and target API 36")
...
if "versionCode = 40" not in build_file:
    errors.append("Android RC5 versionCode must be 40")
if 'setOf("arm64-v8a", "x86_64")' not in build_file:
    errors.append("Android public packages must be limited to 64-bit ABIs")
if "jniLibs.useLegacyPackaging = false" not in build_file:
    errors.append("Native libraries must use modern uncompressed packaging")
