...
if 'versionName = "3.0.0-rc.1"' not in build_file:
    errors.append("versionName must be 3.0.0-rc.1 for this release")
if 'buildConfigField("String", "RELEASE_VERSION", "\\"3.0.0-rc.1\\"")' not in build_file:
    errors.append("RELEASE_VERSION must be 3.0.0-rc.1 for this release")
...
if not re.search(r"^\s*versionCode\s*=\s*62\s*$", build_file, re.MULTILINE):
    errors.append("Android 2.0.6 stable versionCode must be 62")
if 'setOf("arm64-v8a", "armeabi-v7a", "x86_64")' not in build_file:
    errors.append("Android public packages must include arm64-v8a, armeabi-v7a and x86_64 ABIs")
