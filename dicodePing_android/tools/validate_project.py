...
if 'versionName = "1.9.0-rc.16"' not in build_file:
    errors.append("versionName must be 1.9.0-rc.16 for this release")
if 'buildConfigField("String", "RELEASE_VERSION", "\\"1.9.0-rc.16\\"")' not in build_file:
    errors.append("RELEASE_VERSION must be 1.9.0-rc.16 for this release")

if "compileSdk = 36" not in build_file or "targetSdk = 36" not in build_file:
    errors.append("Android RC16 must compile and target API 36")
...
if not re.search(r"^\s*versionCode\s*=\s*51\s*$", build_file, re.MULTILINE):
    errors.append("Android RC16 versionCode must be 51")
