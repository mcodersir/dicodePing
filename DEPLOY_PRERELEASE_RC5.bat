...
chcp 65001 >nul
title dicodePing v1.9.0 RC5 - Release Recovery Deploy
cd /d "%~dp0"
...
set "BRANCH=main"
set "TAG=v1.9.0-rc.5"
set "WORKFLOW=v1.9.0-rc.5-release.yml"
set "TRIGGER_REL=.github\release-triggers\v1.9.0-rc.5.txt"
set "COMMIT_MESSAGE=release: recover and publish v1.9.0-rc.5 prerelease"
set "SOURCE_DIR=%CD%"
set "STAGE_DIR=%TEMP%\dicodePing-deploy-v190-rc5-%RANDOM%%RANDOM%"
set "WAIT_SCRIPT=%SOURCE_DIR%\tools\wait_for_github_release_rc5.ps1"
set "HEAD_SHA="
set "GIT_USER="
...
cls
echo ================================================================
echo      dicodePing v1.9.0 RC5 - Release Recovery Deploy
echo ================================================================
echo.
echo This script uses Git for repository authentication and deployment.
echo An existing partial RC5 release is updated in place after a successful build.
echo.
echo Steps:
...
echo   3. Copy and validate the fixed RC5 source.
echo   4. Commit and push a unique release trigger to main.
echo   5. Move the RC5 tag to the fixed commit when GitHub permits it.
...
if not exist "docs\releases\v1.9.0-rc.5.md" (
    echo [ERROR] Release notes were not found:
    echo         docs\releases\v1.9.0-rc.5.md
    goto :failed
)
...
echo [2/7] Checking the current RC5 tag and release state...
...
if defined RELEASE_EXISTS (
    ...
    echo The RC5 tag and release slot are available.
)
popd

echo [3/7] Copying the fixed RC5 source into the clean repository clone...
...
echo [6/7] Pointing %TAG% at the fixed RC5 commit...
