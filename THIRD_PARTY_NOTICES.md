...
The RC6 scanner verification flow is adapted from the public `mcodersir/DicodeConfigChecker` project: supported configurations are tested by starting an Xray outbound and performing real HTTP traffic through it, while unsupported formats use a bounded TCP fallback. The RC6 implementation was rewritten for dicodePing's worker, cancellation, persistence and UI contracts.
