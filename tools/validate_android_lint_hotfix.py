...
def main() -> int:
    ...
    if "MaterialSwitch" not in item_source or "<Switch" in item_source:
        errors.append("Legacy platform Switch remains in item_source.xml")
    ...
    if errors:
        print("Android stable lint hotfix validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("Android stable lint/build hotfix validation passed")
    return 0
