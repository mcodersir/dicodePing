...
def main() -> int:
    ...
    require('VERSION = "2.0.1"' in constants, "Stable VERSION is not 2.0.1", errors)
    require('RELEASE_VERSION = "2.0.1"' in constants, "Stable RELEASE_VERSION is not 2.0.1", errors)
    require('__version__ = "2.0.1"' in init, "Python package version is not stable 2.0.1", errors)
    require('versionCode = 57' in gradle, "Android stable versionCode is not 57", errors)
    require('versionName = "2.0.1"' in gradle, "Android stable versionName is incorrect", errors)
    require('buildConfigField("String", "RELEASE_VERSION", "\\"2.0.1\\"")' in gradle, "Android RELEASE_VERSION is incorrect", errors)

    for builder in ("tools/build_windows.py", "tools/build_linux.py", "tools/build_macos.py"):
        body = read(builder)
        require('APP_VERSION = "2.0.1"' in body, f"{builder} does not build stable 2.0.1", errors)
    ...
    # v2.0.1: scanner splits save from enrichment. The inline post-save
    ...
    require("def enrich_saved_scanner_records" in scanner, "Desktop v2.0.1 public enrichment entry point is missing", errors)
    ...
    require("SCANNER_PROBE_CONCURRENCY = 3" in android_repo, "Android scanner probe concurrency was not bumped to 3", errors)
    require("fun enrichScannerRecords" in android_repo, "Android v2.0.1 public enrichment entry point is missing", errors)
    ...
    require("2.0.1 پایدار" in site, "Website is not labeled as stable", errors)
    require("releases/download/v2.0.1/" in site, "Website download links do not target v2.0.1", errors)
    ...
    require("python tools/validate_v201_stable.py" in release, "Release workflow does not run the stable validator", errors)
    ...
    require("set \"TAG=v2.0.1\"" in deployer, "Deployer tag is not v2.0.1", errors)
    ...
    require("tools\\validate_v201_stable.py" in deployer, "Deployer does not run the v2.0.1 validator", errors)
    ...
    if errors:
        print("dicodePing 2.0.1 stable validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("dicodePing 2.0.1 stable release validation passed")
    return 0
