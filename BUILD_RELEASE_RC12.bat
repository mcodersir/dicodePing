...
call :run tools\verify_version.py --tag v1.9.0-rc.12
if errorlevel 1 goto failed
...
echo.
echo ==============================================================
echo RC12 legacy-style Windows build completed:
echo   release\dicodePing-v1.9.0-rc.12-windows-x64.exe
echo.
echo Android signed APK:
echo   BUILD_SIGNED_APK_RC12.bat
echo.
echo Windows + Linux + macOS + Android release assets:
echo   Run workflow: .github\workflows\v1.9.0-rc.12-release.yml
echo ==============================================================
pause
exit /b 0
...
:failed
echo.
echo RC12 legacy-style build failed. Read the first error above.
pause
exit /b 1
