...
call :run tools\verify_version.py --tag v1.9.0-rc.7
if errorlevel 1 goto failed
...
echo.
echo ==============================================================
echo RC7 legacy-style Windows build completed:
echo   release\dicodePing-v1.9.0-rc.7-windows-x64.exe
echo.
echo Android signed APK:
echo   BUILD_SIGNED_APK_RC7.bat
echo.
echo Windows + Linux + macOS + Android release assets:
echo   Run workflow: .github\workflows\v1.9.0-rc.7-release.yml
echo ==============================================================
pause
exit /b 0
...
:failed
echo.
echo RC7 legacy-style build failed. Read the first error above.
pause
exit /b 1
