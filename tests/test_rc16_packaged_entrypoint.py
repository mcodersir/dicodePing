...
def test_all_desktop_builders_package_the_rc16_runtime_wrapper() -> None:
    for relative in (
        ...
        assert 'entrypoint = root / "app_v190_rc16.py"' in builder
        assert 'entrypoint = root / "app.py"' not in builder

    spec = read("dicodePing.spec")
    assert '[str(root / "app_v190_rc16.py")]' in spec


def test_rc16_wrapper_installs_discovery_runtime_before_importing_app() -> None:
    wrapper = read("app_v190_rc16.py")
