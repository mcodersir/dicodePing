...
def test_android_crawler_uses_bounded_fast_worker_queue() -> None:
    ...
    assert "retryOnConnectionFailure(true)" in source
    assert "followRedirects(true)" in source
...
def test_android_scanner_progress_is_monotonic_and_target_aware() -> None:
    ...
    assert "channelRatio * 0.70 + configRatio * 0.30" in source
...
def test_rc13_release_files_exist() -> None:
