...
def test_version_and_android_code():
    assert 'RELEASE_VERSION = "1.9.0-rc.14"' in text('dicodeping/constants.py')
    gradle=text('dicodePing_android/app/build.gradle.kts')
    assert 'versionCode = 49' in gradle
    assert 'versionName = "1.9.0-rc.14"' in gradle
...
def test_pages_workflow_versions_and_shape():
    ...
    assert 'actions/upload-pages-artifact@v3' in docs
