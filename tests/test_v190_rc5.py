...
def test_rc5_release_metadata_and_deployer_are_consistent() -> None:
    assert 'RELEASE_VERSION = "1.9.0-rc.13"' in read("dicodeping/constants.py")
    assert '__version__ = "1.9.0rc13"' in read("dicodeping/__init__.py")
    deploy = read("DEPLOY_PRERELEASE_RC13.bat")
    assert "v1.9.0-rc.13" in deploy
    assert "v1.9.0-rc.13-release.yml" in deploy
