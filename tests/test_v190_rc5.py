...
def test_desktop_scanner_layout_and_disconnect_lifecycle_are_safe() -> None:
    ...
    rc6 = read("dicodeping/rc6_runtime.py")
    ...
    assert "requestInterruption()" in rc6
    assert "worker.finished.connect" in rc6
    assert "scanner.finished.connect" in rc6
    assert "monitor.finished.connect" in rc6
    assert ".terminate()" not in rc6
