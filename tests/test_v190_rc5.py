...
def test_rc5_release_metadata_and_deployer_are_consistent() -> None:
    assert 'RELEASE_VERSION = "1.9.0-rc.10"' in read("dicodeping/constants.py")
    assert '__version__ = "1.9.0rc10"' in read("dicodeping/__init__.py")
    deploy = read("DEPLOY_PRERELEASE_RC10.bat")
    assert "v1.9.0-rc.10" in deploy
    assert "v1.9.0-rc.10-release.yml" in deploy
