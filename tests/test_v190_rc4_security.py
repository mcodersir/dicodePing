...
def test_only_android_release_apk_builder_requires_owner_signing_key() -> None:
    root_builder = (ROOT / "BUILD_SIGNED_APK_RC13.bat").read_text("utf-8")
    apk_builder = (ROOT / "dicodePing_android/build_apk_rc13.bat").read_text("utf-8")
    debug_builder = (ROOT / "dicodePing_android/build_debug_apk.bat").read_text("utf-8")
    gradle = (ROOT / "dicodePing_android/app/build.gradle.kts").read_text("utf-8")

    assert "dicodePing_android\\build_apk_rc13.bat" in root_builder
    ...
    assert "dicodePing-v1.9.0-rc.13-android.apk" in apk_builder
...
def test_ci_publishes_the_same_asset_shape_as_v180_rc4_plus_macos() -> None:
    workflow = (ROOT / ".github/workflows/v1.9.0-rc.13-release.yml").read_text("utf-8")
    shell_builder = (ROOT / "dicodePing_android/build_apk_rc13.sh").read_text("utf-8")

    assert "dicodePing-v1.9.0-rc.13-windows-x64.exe" in workflow
    assert "dicodePing-v1.9.0-rc.13-linux-x86_64.tar.gz" in workflow
    assert "dicodePing-v1.9.0-rc.13-macos-${{ matrix.architecture }}.dmg" in workflow
    assert "dicodePing-v1.9.0-rc.13-android.apk" in workflow
    assert "dicodePing-core-aether" not in workflow
    assert "dicodePing-core-usque" not in workflow
    assert "prepare_bundled_cores.py" in workflow
    ...
    assert "dicodePing-v1.9.0-rc.13-android.apk" in shell_builder
