...
def test_only_android_release_apk_builder_requires_owner_signing_key() -> None:
    root_builder = (ROOT / "BUILD_SIGNED_APK_RC7.bat").read_text("utf-8")
    apk_builder = (ROOT / "dicodePing_android/build_apk_rc7.bat").read_text("utf-8")
    debug_builder = (ROOT / "dicodePing_android/build_debug_apk.bat").read_text("utf-8")
    gradle = (ROOT / "dicodePing_android/app/build.gradle.kts").read_text("utf-8")

    assert "dicodePing_android\\build_apk_rc7.bat" in root_builder
    ...
    assert "dicodePing-v1.9.0-rc.7-android.apk" in apk_builder
...
def test_ci_publishes_the_same_asset_shape_as_v180_rc4_plus_macos() -> None:
    ...
    shell_builder = (ROOT / "dicodePing_android/build_apk_rc7.sh").read_text("utf-8")

    assert "dicodePing-v1.9.0-rc.7-windows-x64.exe" in workflow
    assert "dicodePing-v1.9.0-rc.7-linux-x86_64.tar.gz" in workflow
    assert "dicodePing-v1.9.0-rc.7-macos-${{ matrix.architecture }}.dmg" in workflow
    assert "dicodePing-v1.9.0-rc.7-android.apk" in workflow
    ...
    assert "dicodePing-v1.9.0-rc.7-android.apk" in shell_builder
