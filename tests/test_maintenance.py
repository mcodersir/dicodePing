...
class MaintenanceTests(unittest.TestCase):
    def test_android_rc19_release_sequence_and_pipeline(self) -> None:
        ...
        self.assertIn("versionCode = 54", gradle)
    ...
    def test_rc13_filter_callbacks_do_not_bind_a_runtime_patched_method(self) -> None:
        ui = (ROOT / "dicodeping/ui.py").read_text(encoding="utf-8")
    ...
    def test_rc13_resize_filter_is_scoped_to_the_main_window(self) -> None:
    ...
    def test_rc19_packaged_desktop_smoke_tests_require_success(self) -> None:
    ...
    def test_rc2_release_tests_packaged_discovery_and_rendering(self) -> None:
        ...
        self.assertIn("dicodePing-v1.9.0-rc.19-windows-x64.exe", workflow)
        self.assertIn("dicodePing-v1.9.0-rc.19-linux-x86_64.tar.gz", workflow)

    def test_windows_protocol_is_not_rendered(self) -> None:
        ui = (ROOT / "dicodeping/ui.py").read_text(encoding="utf-8")
        self.assertNotIn("server.protocol", ui)
        self.assertIn("self.table = QTableWidget(0, 9)", ui)
    ...
    def test_connection_stability_tuning_is_present(self) -> None:
        ...
        self.assertIn("START_NOT_STICKY", android_service)
        self.assertNotIn("START_REDELIVER_INTENT", android_service)
    ...
    def test_source_repository_does_not_track_native_release_artifacts(self) -> None:
        ...
        if (ROOT / ".git").exists():
            ...
            # Source ZIPs are mutable working trees: verified cores are created
            # during a build and must not make the second build fail. The
            # immutable source manifest represents the files originally
            # published, not files generated by a previous build.
            source_manifest = ROOT / ".source-files.txt"
            if source_manifest.is_file():
                ...
                ignored_prefixes = ("build/", "dist/", "release/", ".pytest_cache/")
                ignored_exact = {
                    "core/xray.exe", "core/xray", "core/wintun.dll",
                    "core/geoip.dat", "core/geosite.dat", "core/aether.exe",
                    "core/aether", "core/usque.exe", "core/usque",
                    "core/bundled-cores.json",
                }
                tracked = {
                    relative
                    ...
                    if relative not in ignored_exact
                    and not relative.startswith(ignored_prefixes)
                    and path.suffix.lower() != ".aar"
                }
        forbidden = {
            "core/xray.exe",
            "core/xray",
