...
3. Read newest posts first, accepting at most four VMess, VLESS, Trojan or Shadowsocks links per channel from the preceding seven days. Undated posts and Telegram MTProto proxies are excluded. Private/unavailable channels count as errors without aborting other channels.
...
5. Replace the independent **استخر کانفیگ** group only after successful validation. Existing user subscriptions remain separate. Cancellation or no accepted candidates preserves the previous pool.
