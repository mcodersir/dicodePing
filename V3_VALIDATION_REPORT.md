# dicodePing 3.0.0-pre.7 Project Validation Report
...
The publisher clones the current default branch into a temporary checkout, then uses `tools/sync_release_tree.py` to replace the checkout from `SOURCE_MANIFEST.sha256`. The sync preserves only `.git`, verifies every source hash before and after copy, and ignores local runtime downloads, build outputs, caches and other files that are not part of the source manifest. It then validates the source and pushes the Version 3 commit to the default branch. The `v3.0.0-pre.7` tag is created only after source validation. `.github/workflows/release-v3.yml` then builds the four platform families and creates the pre-release with matching assets. `gh.exe` is optional and is used only to watch/verify the workflow when it is already installed and authenticated.
