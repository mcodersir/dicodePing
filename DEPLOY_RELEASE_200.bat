...
set "COMMIT_MESSAGE=release: publish dicodePing v2.0.0 stable"
set "SOURCE_DIR=%CD%"
...
if not exist "docs\releases\v2.0.0.md" goto :missing_source
...
if not exist "tests\test_v200_stable.py" (popd & goto :cleanup_failed)
if not exist "tools\validate_v200_stable.py" (popd & goto :cleanup_failed)
findstr /C:"versionCode = 56" "dicodePing_android\app\build.gradle.kts" >nul || (popd & goto :cleanup_failed)
...
if errorlevel 1 goto :validation_failed
call %PYTHON_CMD% tools\validate_v200_stable.py
if errorlevel 1 goto :validation_failed
