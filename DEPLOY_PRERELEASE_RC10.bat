...
chcp 65001 >nul
title dicodePing v1.9.0 RC10 Scanner Recovery Deploy
cd /d "%~dp0"

set "REPO=mcodersir/dicodePing"
set "REPO_URL=https://github.com/%REPO%.git"
set "BRANCH=main"
set "TAG=v1.9.0-rc.10"
set "WORKFLOW=v1.9.0-rc.10-release.yml"
set "TRIGGER_REL=.github\release-triggers\v1.9.0-rc.10.txt"
set "COMMIT_MESSAGE=fix: publish v1.9.0-rc.10 scanner transport and state repair"
set "SOURCE_DIR=%CD%"
set "STAGE_DIR=%TEMP%\dicodePing-deploy-v190-rc10-%RANDOM%%RANDOM%"
set "WAIT_SCRIPT=%SOURCE_DIR%\tools\wait_for_github_release_rc10.ps1"
set "HEAD_SHA="
set "GIT_USER="
...
cls
echo ================================================================
echo      dicodePing v1.9.0 RC10 Scanner Recovery Deploy
echo ================================================================
echo.
echo This script uses Git for repository authentication and deployment.
echo An existing partial RC10 release is updated in place after a successful build.
echo.
echo Steps:
...
echo   3. Copy and validate the RC10 source.
echo   4. Commit and push a unique release trigger to main.
echo   5. Move the RC10 tag to the fixed commit when GitHub permits it.
...
if not exist "docs\releases\v1.9.0-rc.10.md" (
    echo [ERROR] Release notes were not found:
    echo         docs\releases\v1.9.0-rc.10.md
    goto :failed
)
...
echo [2/7] Checking the current RC10 tag and release state...
...
if defined RELEASE_EXISTS (
    ...
    echo The RC10 tag and release slot are available.
)
popd

echo [3/7] Copying the fixed RC10 source into the clean repository clone...
...
echo [6/7] Pointing %TAG% at the fixed RC10 commit...
