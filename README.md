...
**v1.9.0-rc.16** — اتصال و لغو واقعی Aether/WARP، ماندگاری دائمی خروجی اسکنر، تست سریع‌تر سرورها و لاگ زنده بدون پرش در Android، Windows، Linux و macOS.
...
برای انتشار کامل RC16 در ویندوز، ZIP را Extract و فایل `DEPLOY_PRERELEASE_RC16.bat` را اجرا کنید. این اسکریپت سورس تمیز را روی `main` می‌فرستد، تگ `v1.9.0-rc.16` را ایجاد یا جایگزین می‌کند و تا انتشار خروجی‌های Windows، Linux، macOS و Android منتظر می‌ماند. راهنمای کوتاه در `DEPLOY_PRERELEASE_RC16_README_FA.txt` قرار دارد.

Workflowهای فعال فقط `ci.yml`، `codeql.yml`، `docs.yml` و `release.yml` هستند. انتشار RC16 با تگ `v1.9.0-rc.16` آغاز می‌شود.

## انتشار یک‌کلیکی RC16

پس از Extract کامل بسته در یک پوشه جدید، `DEPLOY_PRERELEASE_RC16.bat` را اجرا کنید. این ابزار Clone، پاک‌سازی فایل‌های RC قبلی، اعتبارسنجی، Push، ساخت مجدد Tag، انتشار Pre-release و بازیابی GitHub Pages را یکجا انجام می‌دهد. جزئیات در `RC16_RELEASE_DEPLOY_FIX_FA.md` آمده است.
