...
- [اسکنر](#اسکنر)
  ...
  - [همزمانی واقعی (v2.0.6)](#%D9%87%D9%85%D8%B2%D9%85%D8%A7%D9%86%DB%8C-%D9%88%D8%A7%D9%82%D8%B9%DB%8C-v206)
...
- **تستر کانفیگ واقعی** — هر کانفیگ با یک پروب TCP و یک درخواست HTTP واقعی از داخل Xray سنجیده می‌شود. پینگ نمایش‌داده‌شده همان تاخیر واقعی صفحه تست اتصال Apple از طریق پروکسی است، نه ICMP یا TCP خام.
...
5. APK شامل `arm64-v8a`، `armeabi-v7a` و `x86_64` است — هم روی گوشی‌های واقعی و هم روی شبیه‌سازها کار می‌کند.
...
1. **فاز A — پروب موازی TCP handshake delay (`parallelTcpProbe`):** ۲۰ سوکت همزمان روی `Dispatchers.IO` به هر کاندید وصل می‌شوند و تاخیر handshake را اندازه می‌گیرند. این کار JNI-safe است چون هر `Socket` یک نمونه مستقل است.
2. **فاز B — پروب سری Xray HTTP روی برترین بازماندگان:** کاندیدهای زنده بر اساس تاخیر TCP مرتب و فقط ۳۲ کاندید برتر (`SCANNER_NATIVE_CANDIDATE_LIMIT`) از طریق هسته Xray تست می‌شوند. چون فاز A میزبان‌های مرده را فیلتر کرده، فاز B در کسری از زمان قبلی تمام می‌شود.
...
**اثر خالص روی یک اسکن ۴۰ کاندیدی:** فاز A معمولاً ۱-۲ ثانیه (موازی)، فاز B حدود ۵-۸ ثانیه (سری اما فقط روی بازماندگان زنده)، مجموع حدود ۶-۱۰ ثانیه به جای ۶۰-۹۰ ثانیه در v2.0.0/2.0.1.
